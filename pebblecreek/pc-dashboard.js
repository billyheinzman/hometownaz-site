...
function renderHeaderKpis(mode) {
  ...
  var kpis = [
    ...
    {n: miFmtMoney(totalVol),              lbl: 'Total Volume',                  tip: 'Cumulative dollar value of every sale tracked. The size of PebbleCreek housing economy.'}
  ];
...
// Strips Robson model patterns (Augusta 1280, Solaris 2160, Bacara 3077, etc.),
...
function miNormalizeModel(s) {
  ...
  if (lower === 'pebblecreek' || lower === 'pebble creek' || lower === 'robson' || lower === '--') return '';
...
function miComputeValuation(params) {
  ...
  if (model) {
    ...
    // Only trust the model if (a) we found enough records AND (b) the model name looks like a real Robson plan name
...
function miLeadCaptureHtml(context) {
  return '<div class="mi-lead-card">' +
    ...
    '<div class="mi-lead-sub">' +
      'Get a customized PebbleCreek market report — including these results, plus comparable sales detail, and Lona\'s expert read on what it means for ' +
...
function miSubmitLead(ctx) {
  ...
  var subject = 'PebbleCreek Market Report Request — ' + name;
  ...
  var mailto = 'mailto:azlonaking@gmail.com?cc=billyheinzman@gmail.com&subject=' +
  ...
  document.getElementById('leadFormWrap-' + ctx).innerHTML =
    '<div class="mi-lead-success">' +
      ...
      '<a href="mailto:azlonaking@gmail.com" style="color:#fff;text-decoration:underline;">azlonaking@gmail.com</a> ' +
...
function renderTimeMachine() {
  ...
  if (pctChange > 50) {
    ...
    html += 'Steady, reliable appreciation typical of well-built Robson floor plans.';
  } else if (pctChange > 0) {
...
function renderCrystalBall() {
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:8px;">📊 Median Sale Price Trajectory — PebbleCreek (with ' + nextY + ' projection)</div>' +
    svg +
  '</div>';
...
function renderPropHistory() {
  ...
  if (!q || q.length < 3) {
    ...
    resEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:24px;text-align:center;color:var(--muted);">' +
      '<div style="font-size:2rem;margin-bottom:8px;">🏘️</div>' +
      '<div style="font-size:13px;">Type any PebbleCreek address to see its complete sale history</div>' +
...
function renderStreetDetail(s) {
  ...
  if (typeof document !== 'undefined' && document.title) {
    document.title = s.name + ' — PebbleCreek Real Estate Profile | Lona King & Billy Heinzman';
  }
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:14px;padding:24px 28px;margin-bottom:14px;position:relative;overflow:hidden;">' +
    ...
    '<div style="position:relative;">' +
      '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--orange);font-weight:800;margin-bottom:4px;">📍 PebbleCreek Street Profile</div>' +
  ...
  html += '<div style="background:linear-gradient(135deg,var(--orange),#e8a040);color:#fff;border-radius:12px;padding:20px 24px;margin-bottom:14px;">' +
    ...
    '<div style="font-size:12px;opacity:.95;line-height:1.5;margin-bottom:12px;">' +
      'Lona King and Billy Heinzman have tracked every closing on ' + s.name + ' for the past 17 years. We know which sales are outliers, which homes have hidden upgrades, and which ones to avoid. Talk to us before you make a move.' +
    '</div>' +
    '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
...
function renderLeaderboard() {
  ...
  resEl.innerHTML = '<div class="coming-soon-card">' +
    ...
    '<div class="coming-soon-sub">' +
      'Once we wire in <strong>Agent Productivity</strong> data from ARMLS, this tab will show the top-performing PebbleCreek listing agents ranked by:' +
    '</div>' +
    '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;max-width:560px;margin:0 auto;">' +
      ...
      '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:left;">' +
        ...
        '<div style="font-size:11px;color:var(--muted);line-height:1.5;">Top agents per Robson model — Solaris, Fresco, etc.</div>' +
      '</div>' +
    '</div>' +
    ...
    '<div style="margin-top:18px;font-size:11px;color:var(--orange);font-weight:700;">' +
      '💡 Once enabled, this tool will be <em>the</em> reason every PebbleCreek agent visits this site weekly.' +
    '</div>' +
  '</div>';
}
...
function renderFloorPlans() {
  ...
  var html = '<div style="font-size:11px;color:var(--muted);margin-bottom:10px;">Showing <strong>' + models.length + '</strong> Robson floor plans · click any card to see all sales of that model</div>';
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:12px;padding:18px 22px;margin-top:18px;">' +
    ...
    '<div style="font-size:12px;opacity:.9;line-height:1.5;margin-bottom:12px;">' +
      'Lona\'s comprehensive PebbleCreek floor plan library has every Robson model with diagrams, sq ft breakdowns, and key features. Currently in development at lonajking.com.' +
    '</div>' +
...
function renderQuarterlyReport() {
  ...
  html += '<div style="border-bottom:3px solid var(--orange);padding-bottom:14px;margin-bottom:24px;">' +
    '<div style="display:flex;justify-content:space-between;align-items:flex-start;">' +
      '<div>' +
        '<div style="font-family:\'Merriweather\',serif;font-size:1.7rem;font-weight:800;color:var(--green);">PebbleCreek Market Report</div>' +
      ...
      '<div style="text-align:right;font-size:11px;color:var(--muted);line-height:1.5;">' +
        ...
        'HomeSmart · PebbleCreek<br>' +
        '(623) 203-6271 · azlonaking@gmail.com<br>' +
        'hometownaz.com' +
      '</div>' +
    '</div>' +
  '</div>';
  ...
  html += '<div style="background:#1d3a2c;color:#fff;border-radius:10px;padding:20px 24px;margin-top:24px;">' +
    '<div style="font-family:\'Merriweather\',serif;font-size:1.1rem;font-weight:700;margin-bottom:6px;color:var(--orange);">Ready to make your move in PebbleCreek?</div>' +
    '<div style="font-size:12px;line-height:1.6;opacity:.9;margin-bottom:10px;">' +
      'This report is generated from real ARMLS closed sales data — not estimates. Lona King brings 20+ years of West Valley real estate experience, including a decade as Senior Contract Director with Del Webb in the West Valley. ' +
      'Billy Heinzman handles the data, the tech, and the marketing strategy. Together, they bring a data-driven approach to PebbleCreek.' +
    '</div>' +
    '<div style="font-size:12px;font-weight:700;">📞 (623) 203-6271 · ✉️ azlonaking@gmail.com · 🌐 hometownaz.com</div>' +
  '</div>';

  // Methodology footnote
  html += '<div style="font-size:10px;color:var(--muted);margin-top:14px;font-style:italic;text-align:center;">' +
    'Source: ARMLS closed sales data, PebbleCreek (zip 85395). Period: ' + periodLabel + '. ' +
