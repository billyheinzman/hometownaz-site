...
//  MARKET INTELLIGENCE DASHBOARD â multi-year comparison + global search
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function miFmtPct(delta) {
  if (delta == null || isNaN(delta)) return 'â';
...
function miDeltaArrow(delta, inverse) {
  if (delta == null || isNaN(delta) || Math.abs(delta) < 0.5) return 'â';
  if (inverse) return delta < 0 ? 'â' : 'â';
  return delta > 0 ? 'â' : 'â';
}
...
// ââ Mode switching ââââââââââââââââââââââââââââââââââââââââââââââââââââ
function showMIMode(mode) {
  ...
  if (mode === 'compare') {
    // Pre-fill A=current tab in 2025, B=current tab in 2026 (if both exist) â most useful default
...
// ââ Header KPIs âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function renderHeaderKpis(mode) {
  ...
  var yrLabel = years.length === 1 ? years[0] : (years[0] + 'â' + years[years.length-1]);

  var kpis = [
    {n: totalCount.toLocaleString(),       lbl: 'Total Sales (' + yrLabel + ')', tip: 'Every closed sale across all years currently loaded â the foundation of every tool here.'},
    {n: miFmtMoney(avgPrice),              lbl: 'Avg Price',                     tip: 'Average closing price across all ' + totalCount.toLocaleString() + ' tracked sales.'},
    {n: '$' + Math.round(avgPsf),          lbl: 'Avg $/Sq Ft',                   tip: 'Average price per square foot â the most reliable cross-model value indicator.'},
    {n: Math.round(avgDom) + ' days',      lbl: 'Avg DOM',                       tip: 'Average days on market from listing to close. Lower = hotter market.'},
    {n: miFmtMoney(totalVol),              lbl: 'Total Volume',                  tip: 'Cumulative dollar value of every sale tracked. The size of the SCW housing economy.'}
  ];
...
// Period definitions â return list of month keys
var PERIOD_DEFS = {
...
function miResolvePeriod(year, period, singleMonth) {
  ...
  if (period === 'ytd') {
    ...
    // We need access to both years to compute this â caller passes other year context via global pickers
    ...
    if (otherYear === year) {
      ...
      return {keys: keys, label: 'YTD ' + (keys.length ? MI_MONTH_SHORT[keys[0]] + 'â' + MI_MONTH_SHORT[keys[keys.length-1]] : '')};
    }
    ...
    return {keys: keysBoth, label: 'YTD ' + (keysBoth.length ? MI_MONTH_SHORT[keysBoth[0]] + 'â' + MI_MONTH_SHORT[keysBoth[keysBoth.length-1]] : 'no overlap')};
  }
...
function renderYoY() {
  ...
  // Build labels
  var aLabel = aYear + ' Â· ' + pa.label;
  var bLabel = bYear + ' Â· ' + pb.label;

  function delta(now, prev) { return prev ? ((now - prev) / prev * 100) : null; }

  // KPI cards
  var kpiCards = [
    {lbl:'Sales Count', va: aAgg?aAgg.count.toLocaleString():'â', vb: bAgg?bAgg.count.toLocaleString():'â', d: aAgg&&bAgg?delta(bAgg.count, aAgg.count):null, inv:false},
    {lbl:'Avg Price',   va: aAgg?miFmtMoney(aAgg.avgPrice):'â',   vb: bAgg?miFmtMoney(bAgg.avgPrice):'â', d: aAgg&&bAgg?delta(bAgg.avgPrice, aAgg.avgPrice):null, inv:false},
    {lbl:'Avg $/Sq Ft', va: aAgg?'$'+Math.round(aAgg.avgPsf):'â', vb: bAgg?'$'+Math.round(bAgg.avgPsf):'â', d: aAgg&&bAgg?delta(bAgg.avgPsf, aAgg.avgPsf):null, inv:false},
    {lbl:'Avg DOM',     va: aAgg?Math.round(aAgg.avgDom)+'d':'â', vb: bAgg?Math.round(bAgg.avgDom)+'d':'â', d: aAgg&&bAgg?delta(bAgg.avgDom, aAgg.avgDom):null, inv:true},
    {lbl:'Total Volume',va: aAgg?miFmtMoney(aAgg.vol):'â',        vb: bAgg?miFmtMoney(bAgg.vol):'â', d: aAgg&&bAgg?delta(bAgg.vol, aAgg.vol):null, inv:false}
  ];
  ...
  document.getElementById('miYoyKpis').innerHTML = kpiCards.map(function(c){
    ...
    var deltaTxt = c.d != null ? (dArrow + ' ' + miFmtPct(c.d) + ' Î') : 'â';
  ...
  // Chart title + legend
  document.getElementById('yoyChartTitle').textContent = 'Trend Comparison â ' + aLabel + ' vs ' + bLabel;
  ...
  // Summary table
  document.getElementById('yoySummaryTitle').textContent = aLabel + ' vs ' + bLabel + ' â Detail';
  var rows = aAgg && bAgg ? [
  ...
  if (!rows.length) {
    ...
    tableHtml = '<div style="display:grid;grid-template-columns:1.6fr 1fr 1fr 0.8fr 1.2fr;gap:0;font-size:12px;">' +
      ...
      '<div style="padding:8px 10px;font-weight:700;color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.06em;border-bottom:2px solid var(--border);text-align:right;">Î</div>' +
    ...
    rows.forEach(function(r){
      ...
      var dTxt = r.d != null ? (dArrow + ' ' + miFmtPct(r.d)) : 'â';
...
function renderTrends() {
  ...
  document.getElementById('trendsSubhead').textContent = 'Click any card to view that month\'s full data Â· ' + year;
  ...
  active.forEach(function(k){
    ...
    cardsHtml += '<div onclick="closeMarketIntel();switchYear(\'' + year + '\');setTimeout(function(){switchTab(\'' + k + '\');},50)" style="background:var(--cream);border-radius:8px;padding:12px;border:2px solid var(--border);cursor:pointer;transition:all .15s;" onmouseover="this.style.borderColor=\'var(--orange)\';this.style.transform=\'translateY(-2px)\'" onmouseout="this.style.borderColor=\'var(--border)\';this.style.transform=\'translateY(0)\'">' +
      ...
      '<div style="font-size:11px;color:var(--muted);">sales Â· ' + m.avg + ' avg</div>' +
      ...
      '<div style="font-size:10px;color:' + domColor + ';margin-top:4px;">â± ' + domNum + ' day avg DOM</div>' +
      '</div>';
  });
  cardsEl.innerHTML = cardsHtml;

  // PROPER bar chart with SVG â no overlapping labels
...
function drawTrendsBar(year, active, ds) {
  ...
  document.getElementById('trendsBarTitle').textContent = '$/Sq Ft by Month â ' + trend + ' Â· ' + year;
...
function renderCompare() {
  ...
  var rows = [
    {lbl:'Sales Count',  va:aData?aData.count.toLocaleString():'â',           vb:bData?bData.count.toLocaleString():'â',           d:aData&&bData?delta(bData.count, aData.count):null,       inv:false},
    {lbl:'Avg Price',    va:aData?miFmtMoney(aData.avgPrice):'â',             vb:bData?miFmtMoney(bData.avgPrice):'â',             d:aData&&bData?delta(bData.avgPrice, aData.avgPrice):null, inv:false},
    {lbl:'Avg $/Sq Ft',  va:aData?'$'+Math.round(aData.avgPsf):'â',           vb:bData?'$'+Math.round(bData.avgPsf):'â',           d:aData&&bData?delta(bData.avgPsf, aData.avgPsf):null,     inv:false},
    {lbl:'Avg DOM',      va:aData?Math.round(aData.avgDom)+' days':'â',       vb:bData?Math.round(bData.avgDom)+' days':'â',       d:aData&&bData?delta(bData.avgDom, aData.avgDom):null,     inv:true},
    {lbl:'Total Volume', va:aData?miFmtMoney(aData.vol):'â',                  vb:bData?miFmtMoney(bData.vol):'â',                  d:aData&&bData?delta(bData.vol, aData.vol):null,           inv:false}
  ];
  ...
  if (aData) {
    html += '<div style="font-family:\'Merriweather\',serif;font-size:2rem;font-weight:700;color:var(--green);margin-bottom:6px;">' + aData.count + ' <span style="font-size:.6em;color:var(--muted);">closings</span></div>' +
      ...
      '<strong>' + miFmtMoney(aData.avgPrice) + '</strong> avg Â· <strong>$' + Math.round(aData.avgPsf) + '</strong>/sf<br>' +
      '<strong>' + Math.round(aData.avgDom) + ' days</strong> avg DOM Â· <strong>' + miFmtMoney(aData.vol) + '</strong> total</div>';
  } else {
  ...
  if (bData) {
    html += '<div style="font-family:\'Merriweather\',serif;font-size:2rem;font-weight:700;color:var(--green);margin-bottom:6px;">' + bData.count + ' <span style="font-size:.6em;color:var(--muted);">closings</span></div>' +
      '<div style="font-size:13px;color:var(--text);line-height:1.7;">' +
      '<strong>' + miFmtMoney(bData.avgPrice) + '</strong> avg Â· <strong>$' + Math.round(bData.avgPsf) + '</strong>/sf<br>' +
      '<strong>' + Math.round(bData.avgDom) + ' days</strong> avg DOM Â· <strong>' + miFmtMoney(bData.vol) + '</strong> total</div>';
  } else {
    html += '<div style="font-style:italic;color:var(--muted);padding:20px 0;">No data</div>';
  }
  html += '</div></div>';

  // Delta table â header is dynamic now (could be YoY, sequential, or random combo)
  var changeLabel = (aY === bY) ? 'Î (same year)' : (aM === bM ? 'YoY Change' : 'Î');
  ...
  rows.forEach(function(r){
    ...
    html += '<div style="display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;border-top:1px solid var(--border);font-size:12px;">' +
      ...
      '<div style="padding:10px 12px;text-align:right;" class="' + dClass + '">' + (r.d != null ? dArrow + ' ' + miFmtPct(r.d) : 'â') + '</div>' +
    '</div>';
  });
  html += '</div>';
...
function buildSearchAll() {
  ...
  var all = [];
  // Iterate every year that has data in the registry â auto-includes 2016-2019 once added
...
function renderInsights() {
  ...
  // Seasonality (2025 only â full data)
  var byMonth25 = {};
  MI_MONTH_KEYS.forEach(function(k){
  ...
  if (a25 && a26 && bothMonths.length) {
    ...
    html += '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px;border-left:4px solid ' + paceColor + ';">' +
      '<div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:8px;">ð Market Pace 2026 vs 2025</div>' +
  ...
  if (maxCountMo && minCountMo) {
    html += '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px;border-left:4px solid var(--orange);">' +
      '<div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:8px;">ð¡ï¸ 2025 Seasonality</div>' +
      '<div style="font-family:\'Merriweather\',serif;font-size:1.3rem;font-weight:700;color:var(--green);margin-bottom:8px;">Peak: ' + MI_MONTH_FULL[maxCountMo] + ' Â· Slow: ' + MI_MONTH_FULL[minCountMo] + '</div>' +
      '<div style="font-size:12px;color:var(--text);line-height:1.7;"><strong>' + MI_MONTH_FULL[maxCountMo] + '</strong> closed <strong>' + byMonth25[maxCountMo].count + '</strong> homes â the busiest month of 2025. <strong>' + MI_MONTH_FULL[minCountMo] + '</strong> was slowest at <strong>' + byMonth25[minCountMo].count + '</strong>. Plan listings around buyer demand.</div></div>';
  }

  // Standout records
  if (biggestSale) {
    html += '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px;border-left:4px solid var(--sage);">' +
      '<div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:8px;">ð° Biggest Sale (2025â2026)</div>' +
      ...
      biggestSale.beds + ' bed / ' + biggestSale.baths + ' bath Â· ' + biggestSale.sqft.toLocaleString() + ' sq ft Â· built ' + biggestSale.yr + ' Â· closed ' + MI_MONTH_FULL[biggestSale.month] + ' ' + biggestSale.year + '</div></div>';
  }
  if (bestPsfDeal) {
    html += '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px;border-left:4px solid #7c5cbf;">' +
      '<div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:8px;">ð¯ Best Value $/Sq Ft (2025â2026)</div>' +
      ...
      'Sold for ' + miFmtMoneyFull(bestPsfDeal.price) + ' Â· ' + bestPsfDeal.sqft.toLocaleString() + ' sq ft Â· built ' + bestPsfDeal.yr + ' Â· closed ' + MI_MONTH_FULL[bestPsfDeal.month] + ' ' + bestPsfDeal.year + '</div></div>';
  }
  ...
  if (bothMonths.length > 0) {
    ...
    html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:0;font-size:11px;">' +
      ...
      '<div style="padding:8px 10px;font-weight:700;font-size:9px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);border-bottom:2px solid var(--border);text-align:right;">Sales Î</div>' +
      '<div style="padding:8px 10px;font-weight:700;font-size:9px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);border-bottom:2px solid var(--border);text-align:right;">$/SF Î</div>';
    bothMonths.forEach(function(k){
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
//  MARKET INTELLIGENCE â 5 NEW FEATURES (Estimator, Time Machine,
...
function miNormalizeModel(s) {
  ...
  x = x.replace(/[-â,/]\s*(expanded|extended|modified|custom|remodeled|expnd|exp|mod|custom)\b.*$/i, '').trim();
  ...
  if (lower === 'sun city west' || lower === 'del webb' || lower === '--') return '';
...
function miComputeValuation(params) {
  ...
  // Recency weighting â newer comps count more
...
function miLeadCaptureHtml(context) {
  return '<div class="mi-lead-card">' +
    '<div class="mi-lead-title">ð¬ Want this in a polished PDF report?</div>' +
    '<div class="mi-lead-sub">' +
      'Get a customized Sun City Festival market report â including these results, plus comparable sales detail, and Lona\'s expert read on what it means for ' +
    ...
    '<div id="leadFormWrap-' + context.replace(/\s+/g,'_') + '">' +
      '<div class="mi-lead-form">' +
        ...
        '<button onclick="miSubmitLead(\'' + context.replace(/\s+/g,'_').replace(/'/g,"\\'") + '\')">Send My Report â</button>' +
      '</div>' +
    '</div>' +
    '</div>';
}
...
function miSubmitLead(ctx) {
  ...
  // Build mailto fallback (no backend required â everything stays client-side)
  var subject = 'Sun City Festival Market Report Request â ' + name;
  ...
  document.getElementById('leadFormWrap-' + ctx).innerHTML =
    '<div class="mi-lead-success">' +
      '<strong>Thanks, ' + name.split(' ')[0] + '! ð</strong><br>' +
...
function renderEstimator() {
  ...
  if (!model && !sqft && !yr) {
    resultEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:20px;text-align:center;color:var(--muted);font-size:13px;">' +
      'ð Pick a model, or enter sq ft / year built to get an instant estimate.</div>';
    leadEl.innerHTML = '';
    return;
  }
  ...
  // Prefer recent sales â weight by recency
  ...
  if (n < 3) {
    resultEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:20px;text-align:center;color:var(--muted);font-size:13px;">' +
      'â ï¸ Not enough comparable sales found. Try widening your criteria.</div>';
    leadEl.innerHTML = '';
    return;
  }
  ...
  // Range: Â±7% for high conf, Â±10% med, Â±15% low
  var bandPct = conf === 'high' ? 0.07 : (conf === 'med' ? 0.10 : 0.15);
  ...
  var html = '<div class="est-result-card">' +
    ...
    '<div class="est-result-range">' + miFmtMoney(lowVal) + ' â ' + miFmtMoney(highVal) + '</div>' +
    '<div class="est-result-mid">Midpoint: ' + miFmtMoney(midValue) + '</div>' +
    '<div class="est-result-band">' +
      '<span style="color:var(--muted);">Based on <strong>' + n + '</strong> comparable sales Â· ' +
      '<strong>' + miFmtMoney(weightedAvgPsf) + '/sf</strong> weighted avg Â· ' +
  ...
  html += '<div style="margin-top:16px;background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px;">' +
    '<div style="font-size:12px;font-weight:700;color:var(--green);margin-bottom:10px;">ð Top 5 Most Comparable Sales</div>' +
...
function renderTimeMachine() {
  ...
  if (!model) {
    resultEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:24px;text-align:center;color:var(--muted);">' +
      ...
      '<div style="font-size:13px;">Pick a model above to see how its value has evolved 2020 â 2026</div></div>';
    return;
  }
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:8px;">ð Median Sale Price by Year â ' + model + '</div>' +
    svg +
  '</div>';
  ...
  if (pctChange > 50) {
    html += '<strong style="color:var(--orange);">ð Massive appreciation.</strong> The ' + model + ' has appreciated <strong>' + Math.round(pctChange) + '%</strong> over ' + yrSpan + ' years â a compound annual rate of ' + annualRate.toFixed(1) + '%. ';
    html += 'Owners who purchased in ' + first.year + ' have seen substantial equity growth.';
  } else if (pctChange > 20) {
    html += '<strong style="color:var(--sage);">ð Solid growth.</strong> The ' + model + ' has appreciated <strong>' + Math.round(pctChange) + '%</strong> across this ' + yrSpan + '-year window â a compound rate of ' + annualRate.toFixed(1) + '% annually. ';
    html += 'Steady, reliable appreciation typical of well-built Del Webb floor plans.';
  } else if (pctChange > 0) {
    html += '<strong style="color:var(--sage);">ð Modest gains.</strong> The ' + model + ' is up <strong>' + Math.round(pctChange) + '%</strong> over ' + yrSpan + ' years. ';
    html += 'Prices held steady through the rate-shock period and are gradually trending upward.';
  } else {
    html += '<strong style="color:#c62828;">ð Pricing pressure.</strong> Median pricing for the ' + model + ' is <strong>' + Math.abs(Math.round(pctChange)) + '%</strong> below the ' + first.year + ' baseline. ';
    html += 'This may signal a buying opportunity â talk to Lona about timing.';
  }
  html += '</div></div>';
...
function renderHeatMap() {
  ...
  all.forEach(function(r){
    ...
    if (scope === 'street') {
      ...
      // e.g. "13606 W ANTELOPE DR" â "Antelope Dr"
  ...
  html += '<div style="background:#fff7ec;border:1px solid var(--orange);border-radius:10px;padding:14px;margin-top:12px;font-size:12px;color:var(--green);line-height:1.7;">' +
    '<strong style="color:var(--orange);">ð¥ Top of the leaderboard:</strong> <strong>' + hottest.key + '</strong> with ' + hottest.n + ' sales, $' + Math.round(hottest.psf) + '/sf average, ' + hottest.dom + '-day average DOM, and ' + miFmtMoney(hottest.median) + ' median price.';
  html += '</div>';
...
function renderShowdown() {
  ...
  if (leftWinner) html += '<div style="text-align:center;background:var(--orange);color:#fff;padding:4px 10px;border-radius:14px;font-size:10px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;display:inline-block;margin:0 auto 10px auto;width:auto;">ð Winner ' + wins1 + 'â' + wins2 + '</div>';
  html += '</div>';
  ...
  if (rightWinner) html += '<div style="text-align:center;background:var(--orange);color:#fff;padding:4px 10px;border-radius:14px;font-size:10px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;display:inline-block;margin:0 auto 10px auto;width:auto;">ð Winner ' + wins2 + 'â' + wins1 + '</div>';
  html += '</div>';
  ...
  if (winnerName) {
    html += '<strong style="color:var(--orange);">ð Verdict:</strong> The <strong>' + winnerName + '</strong> takes ' + Math.max(wins1, wins2) + ' of ' + battles.length + ' categories. ';
  } else {
    html += '<strong style="color:var(--orange);">ð¤ It\'s a tie!</strong> Both models are evenly matched. ';
  }
...
function renderCrystalBall() {
  ...
  if (medianGrowth > 4 && domChange < 5) {
    posture = 'ð Sellers\' Market Strengthening';
    postureColor = '#2e7d32';
    ...
    posture = 'ð Buyer\'s Window Opening';
    postureColor = '#c62828';
    postureNarrative = 'Prices flat to declining and DOM rising â buyers gain leverage. Sellers should price aggressively and stage well.';
  } else {
    posture = 'ð Market in Transition';
    postureColor = '#1976d2';
    postureNarrative = 'Mixed signals â pricing and pace diverging. Watch closely; this is the kind of market where pricing strategy makes the biggest difference.';
  }
  ...
  // ââ Build the visualization ââ
  var html = '';
  ...
  html += '<div class="cb-prediction">' +
    ...
    '<div class="cb-pred-range">' + miFmtMoney(medianLow) + ' â ' + miFmtMoney(medianHigh) + '</div>' +
  ...
  // ââ Build a regression chart showing actual + projection ââ
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:8px;">ð Median Sale Price Trajectory â Sun City Festival (with ' + nextY + ' projection)</div>' +
    svg +
  '</div>';

  // Strategic recommendations
  html += '<div style="background:#1d3a2c;color:#fff;border-radius:12px;padding:18px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--orange);margin-bottom:10px;">ð¡ What This Means For You</div>';
  ...
  if (medianGrowth > 3) {
    html += 'Prices are climbing â but don\'t over-price. Hot markets reward properly-priced homes with multiple offers in week 1. Pricing 2-3% above market just adds DOM.';
  ...
  if (medianGrowth > 3) {
    ...
    html += 'Take your time, but don\'t lowball. The market is balanced â well-priced homes still move. Focus on long-term fit over chasing the absolute bottom.';
  } else {
  ...
  // Disclaimer
  html += '<div style="font-size:10px;color:var(--muted);margin-top:8px;text-align:center;font-style:italic;">' +
    'Projection uses linear regression on the most recent ' + fitYears.length + ' years of actual median price data (' + fitYears[0] + 'â' + fitYears[fitYears.length-1] + '). ' +
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
//  PRO TOOLS â 6 NEW REALTOR-FACING FEATURES
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

// âââ Property index by canonical address ââââââââââââââââââââââââââââââ
...
function renderPropHistory() {
  ...
  if (!q || q.length < 3) {
    ...
    resEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:24px;text-align:center;color:var(--muted);">' +
      '<div style="font-size:2rem;margin-bottom:8px;">ðï¸</div>' +
      '<div style="font-size:13px;">Type any Sun City Festival address to see its complete sale history</div>' +
  ...
  if (matches.length === 1 || exactMatch) {
    ...
    sugEl.innerHTML = matches.map(function(b){
      ...
      return '<div class="ph-suggestion" onclick="selectProperty(\'' + b.key.replace(/'/g, "\\'") + '\')">' +
        ...
        '<div class="ph-meta">' + b.salesCount + ' sale' + (b.salesCount===1?'':'s') + ' on file Â· last: ' + miFmtMoneyExact(last.price) + ' (' + last.month.charAt(0).toUpperCase()+last.month.slice(1,3) + " '" + last.year.slice(2) + ')</div>' +
      '</div>';
    }).join('');
    ...
    resEl.innerHTML = '<div style="padding:20px;color:var(--muted);font-size:13px;text-align:center;background:#fdf9f1;border-radius:10px;">' +
      'â ï¸ No matching properties found. Try a partial address or street name.</div>';
  }
}

// Jump from Property History â Estimator with this property's specs pre-filled
...
// Jump from Property History â Time Machine for this property's model
...
function showPropertyDetail(prop) {
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:12px;padding:20px 24px;margin-top:14px;">' +
    ...
    '<div style="font-size:12px;opacity:.8;">' + last.beds + ' bed Â· ' + last.baths + ' bath Â· ' + last.sqft.toLocaleString() + ' sf Â· built ' + last.yr + (last.model || last.name ? ' Â· ' + (last.model || last.name) : '') + '</div>' +
  '</div>';
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:18px 22px;">' +
    '<div style="font-size:12px;font-weight:700;color:var(--green);margin-bottom:12px;">ð Sale History Timeline</div>' +
  ...
  for (var i = sales.length - 1; i >= 0; i--) {
    ...
    if (prev) {
      ...
      sinceLast = '<div style="margin-top:6px;font-size:11px;color:' + (gain >= 0 ? 'var(--sage)' : '#c62828') + ';font-weight:700;">' +
        (gain >= 0 ? 'â +' : 'â ') + miFmtMoney(Math.abs(gain)) + ' (' + (gainPct >= 0 ? '+' : '') + gainPct.toFixed(1) + '%) over ' + yrBetween + ' yr' + (yrBetween===1?'':'s') + ' since prior sale</div>';
    }
    ...
    html += '<div class="ph-event">' +
      ...
      '<div class="ph-event-meta">' +
        '$' + Math.round(s.psf) + '/sf Â· ' + s.dom + ' days on market Â· ' + s.type +
        (s.model || s.name ? ' Â· ' + (s.model || s.name) : '') +
      '</div>' +
      sinceLast +
    '</div>';
  }
  ...
  // âââ Current Value Estimate (uses the same valuation engine as the Estimator tab) âââ
    ...
    html += '<div style="margin-top:18px;background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:14px;padding:22px 26px;position:relative;overflow:hidden;">' +
      ...
      '<div style="position:relative;">' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">' +
          '<span style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:var(--orange);">ð Estimated Today</span>' +
        ...
        '<div style="font-family:\'Merriweather\',serif;font-size:1.9rem;font-weight:800;line-height:1.1;margin-bottom:4px;">' +
          miFmtMoney(valuation.low) + ' â ' + miFmtMoney(valuation.high) +
        '</div>' +
        '<div style="font-size:13px;opacity:.9;margin-bottom:10px;">' +
          'Midpoint: <strong>' + miFmtMoney(valuation.mid) + '</strong>' +
          ' Â· vs last sale of ' + miFmtMoneyExact(lastSalePrice) + ' (' + lastSale.month.charAt(0).toUpperCase()+lastSale.month.slice(1,3) + " '" + lastSale.year.slice(2) + ')' +
        ...
        '<div style="font-size:11px;opacity:.75;margin-bottom:14px;line-height:1.5;">' +
          ...
          '(weighted avg <strong>$' + Math.round(valuation.weightedPsf) + '/sf</strong> Ã ' + Math.round(valuation.subjectSqft).toLocaleString() + ' sf). ' +
          'Newer comps weighted heavier.' +
        '</div>' +
        '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
          '<button onclick="phJumpToEstimator(\'' + (modelKey || '').replace(/\'/g, "\\\'") + '\', ' + lastSale.sqft + ', ' + lastSale.yr + ', ' + lastSale.beds + ', ' + lastSale.baths + ')" style="padding:9px 16px;background:var(--orange);color:#fff;border:none;border-radius:6px;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;">ð  Refine This Estimate â</button>' +
          (modelKey ? '<button onclick="phJumpToTimeMachine(\'' + modelKey.replace(/\'/g, "\\\'") + '\')" style="padding:9px 16px;background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.3);border-radius:6px;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;">â±ï¸ See ' + modelKey + ' Time Machine</button>' : '') +
        '</div>' +
        '<div style="font-size:10px;opacity:.6;margin-top:10px;font-style:italic;line-height:1.5;">' +
          'â ï¸ <strong>Estimate, not an appraisal.</strong> Actual market value depends on condition, upgrades, location specifics, and current demand â talk to Lona for a precise valuation.' +
        '</div>' +
      '</div>' +
    '</div>';
  }
...
// "13606 W ANTELOPE DR" â "Antelope Dr"
// "22008 N Via Montoya"  â "Via Montoya"
...
function renderStreetIndex() {
  ...
  var html = '<div style="font-size:11px;color:var(--muted);margin-bottom:10px;">Showing <strong>' + totalStreets + '</strong> streets with 3+ sales Â· <strong>' + totalSales.toLocaleString() + '</strong> total tracked sales Â· click any street for full profile</div>';
  ...
  filtered.forEach(function(s){
    html += '<div class="street-card" onclick="streetJumpTo(\'' + s.slug + '\')">' +
      ...
      '<div class="street-card-stats">' +
        '<div>ð <strong>' + s.n + '</strong> tracked sale' + (s.n===1?'':'s') + '</div>' +
        '<div>ð° ' + miFmtMoney(s.median) + ' median Â· $' + Math.round(s.psf) + '/sf</div>' +
        '<div>ð avg ' + Math.round(s.sqft).toLocaleString() + ' sf Â· built ' + s.yr + '</div>' +
        '<div>â± avg ' + s.dom + ' days on market</div>' +
      '</div>' +
      '<div class="street-card-cta">View Profile â</div>' +
    '</div>';
  });
  html += '</div>';
...
function renderStreetDetail(s) {
  ...
  if (typeof document !== 'undefined' && document.title) {
    document.title = s.name + ' â Sun City Festival Real Estate Profile | Lona King & Billy Heinzman';
  }
  ...
  html += '<div style="margin-bottom:14px;">' +
    '<button onclick="streetBackToIndex()" style="background:none;border:1px solid var(--border);color:var(--muted);padding:6px 12px;border-radius:6px;font-family:inherit;font-size:11px;cursor:pointer;font-weight:600;">â Back to all streets</button>' +
  '</div>';
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:14px;padding:24px 28px;margin-bottom:14px;position:relative;overflow:hidden;">' +
    ...
    '<div style="position:relative;">' +
      '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--orange);font-weight:800;margin-bottom:4px;">ð Sun City Festival Street Profile</div>' +
      '<div style="font-family:\'Merriweather\',serif;font-size:1.9rem;font-weight:800;line-height:1.1;margin-bottom:6px;">' + s.name + '</div>' +
      '<div style="font-size:13px;opacity:.9;">' + s.n + ' tracked closings Â· median ' + miFmtMoney(s.median) + ' Â· $' + Math.round(s.psf) + '/sf Â· ' + s.dom + ' day avg DOM</div>' +
    '</div>' +
  '</div>';
  ...
  if (yearStats.length >= 3) {
    ...
    html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
      '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:8px;">ð ' + s.name + ' â Median Sale Price by Year</div>' +
      svg +
    '</div>';
  }
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:10px;">ð Model Mix</div>';
  if (topModels.length) {
  ...
  // Year-built distribution
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:10px;">ðï¸ Year Built Distribution</div>';
  ...
  // Top 5 sales
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:10px;">ð Top 5 Sales on ' + s.name + '</div>' +
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:10px;">ð Most Recent Activity</div>' +
    '<table style="width:100%;font-size:12px;border-collapse:collapse;">' +
      '<thead><tr style="text-align:left;color:var(--muted);background:#fdf9f1;">' +
  ...
  recent.forEach(function(r){
    html += '<tr style="border-bottom:1px solid #f0ebe0;">' +
      ...
      '<td style="padding:8px;text-align:center;">' + r.sqft.toLocaleString() + ' sf Â· $' + Math.round(r.psf) + '</td>' +
      '<td style="padding:8px;text-align:center;">' + r.dom + 'd</td>' +
    '</tr>';
  });
  html += '</tbody></table></div>';

  // Lona CTA â context-specific
  html += '<div style="background:linear-gradient(135deg,var(--orange),#e8a040);color:#fff;border-radius:12px;padding:20px 24px;margin-bottom:14px;">' +
    ...
    '</div>' +
    '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
      '<a href="tel:+16232036271" style="display:inline-block;padding:9px 18px;background:#fff;color:var(--orange);border-radius:6px;font-family:inherit;font-size:12px;font-weight:800;text-decoration:none;">ð Call Lona â (623) 203-6271</a>' +
      '<a href="mailto:lona@hometownaz.com?subject=Question%20about%20' + encodeURIComponent(s.name) + '" style="display:inline-block;padding:9px 18px;background:rgba(255,255,255,.2);color:#fff;border:1px solid rgba(255,255,255,.4);border-radius:6px;font-family:inherit;font-size:12px;font-weight:700;text-decoration:none;">âï¸ Email Us</a>' +
    '</div>' +
  '</div>';
...
function renderPricingPower() {
  ...
  if (pool.length < 5) {
    resEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:24px;text-align:center;color:var(--muted);font-size:13px;">' +
      'â ï¸ Not enough comparable sales (' + pool.length + ' found). Widen criteria or extend the recency window.</div>';
    return;
  }
  ...
  var scenarios = [
    { label: 'Aggressive', deltaPct: -7,  desc: 'Below market â fast sale' },
    { label: 'Below Market', deltaPct: -3, desc: 'Soft pricing â multi-offer' },
    { label: 'Market', deltaPct: 0, desc: 'Median for comps' },
    { label: 'Above Market', deltaPct: 3, desc: 'Stretch pricing' },
    { label: 'Premium', deltaPct: 7, desc: 'Top of range â patient' },
  ];
  ...
  scenarios.forEach(function(s, i){
    ...
    html += '<div class="pp-scenario' + (isRec ? ' recommended' : '') + '">' +
      (isRec && i === 2 ? '<div class="pp-recommended-badge">â­ Sweet Spot</div>' : '') +
  ...
  html += '<div style="background:linear-gradient(135deg,#1d3a2c,#2d5040);color:#fff;border-radius:12px;padding:18px 22px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--orange);margin-bottom:8px;">ð¡ Pricing Strategy Read</div>' +
    '<div style="font-size:12px;line-height:1.7;opacity:.95;">';
  if (psfDomFit.slope > 0.3) {
    html += 'Strong positive slope between $/sf and DOM â <strong>this segment is price-sensitive right now</strong>. Each $5/sf above market roughly adds ' + Math.round(psfDomFit.slope * 5) + ' days on market. Pricing below median is rewarded with multi-offers.';
  } else if (psfDomFit.slope > 0.1) {
    html += 'Moderate price-sensitivity â homes priced at or slightly below market sell faster, but premium pricing isn\'t catastrophic. <strong>Median pricing is the safe play.</strong>';
  } else if (psfDomFit.slope > -0.1) {
    html += 'Price elasticity is weak â homes at every price point in this segment have similar DOM. <strong>The market is absorbing inventory uniformly</strong>; pricing strategy matters less than presentation.';
  } else {
    html += 'Surprising negative slope â higher-priced homes in this segment actually sell <em>faster</em>. Likely indicates premium-condition or premium-location homes are clearing the market while average-condition homes linger. <strong>Condition and presentation matter more than price.</strong>';
  }
  html += '</div></div>';
...
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
//  ð AGENT LEADERBOARD â placeholder (data not in current PDFs)
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function renderLeaderboard() {
  ...
  resEl.innerHTML = '<div class="coming-soon-card">' +
    ...
    '<div class="coming-soon-title">Coming Soon â Agent Leaderboard</div>' +
    '<div class="coming-soon-sub">' +
      'Once we wire in <strong>Agent Productivity</strong> data from ARMLS, this tab will show the top-performing Sun City Festival listing agents ranked by:' +
    '</div>' +
    '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;max-width:560px;margin:0 auto;">' +
      '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:left;">' +
        '<div style="font-weight:800;color:var(--green);margin-bottom:6px;">ð Volume Leaders</div>' +
        '<div style="font-size:11px;color:var(--muted);line-height:1.5;">Total closed sales, total dollar volume, market share.</div>' +
      '</div>' +
      '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:left;">' +
        '<div style="font-weight:800;color:var(--green);margin-bottom:6px;">â¡ Speed Leaders</div>' +
        '<div style="font-size:11px;color:var(--muted);line-height:1.5;">Lowest average days-on-market across closed listings.</div>' +
      '</div>' +
      '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:left;">' +
        '<div style="font-weight:800;color:var(--green);margin-bottom:6px;">ð° Pricing Leaders</div>' +
        '<div style="font-size:11px;color:var(--muted);line-height:1.5;">Highest list-to-sale ratio (closest to or above asking).</div>' +
      '</div>' +
      '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:14px;text-align:left;">' +
        '<div style="font-weight:800;color:var(--green);margin-bottom:6px;">ð Specialists</div>' +
        '<div style="font-size:11px;color:var(--muted);line-height:1.5;">Top agents per Del Webb model â Cromwell, Safford, etc.</div>' +
      '</div>' +
    '</div>' +
    '<div class="coming-soon-data-need">' +
      '<strong>ð Data needed:</strong> ARMLS "Agent Productivity Report" (CSV/PDF).<br>' +
    ...
    '<div style="margin-top:18px;font-size:11px;color:var(--orange);font-weight:700;">' +
      'ð¡ Once enabled, this tool will be <em>the</em> reason every Sun City Festival agent visits this site weekly.' +
    '</div>' +
  '</div>';
}

// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
//  ð¤ CONCESSION TRACKER â placeholder (data not in current PDFs)
// ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

function renderConcessions() {
  var resEl = document.getElementById('ccResult');

  // We DO have list price patterns we can synthesize from the data â let's at least show DOM-based stress signals as a proxy for "pricing pressure"
  var all = buildSearchAll();
  var recent24 = all.filter(function(r){
    // Find latest year, go back 24 months
    return true; // simplified â show all-time pressure
  });

  // Compute % of homes selling slow (60+ DOM) per year â proxy for buyer leverage / concession pressure
  var byYear = {};
  all.forEach(function(r){
  ...
  var html = '<div class="coming-soon-card" style="margin-bottom:14px;">' +
    '<div class="coming-soon-icon">ð¤</div>' +
    '<div class="coming-soon-title">Coming Soon â Full Concession Tracker</div>' +
    '<div class="coming-soon-sub">' +
      'Full concession data â including dollar amounts, frequency by price tier, trending direction, and which agents/brokerages negotiate the most concessions â requires a different ARMLS export with transaction details.' +
    '</div>' +
    '<div class="coming-soon-data-need">' +
      '<strong>ð Data needed:</strong> ARMLS Closed Sales report with seller-concession field included.<br>' +
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:18px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:4px;">ð Buyer Leverage Index â Available Now (Proxy)</div>' +
  ...
  html += '<div style="margin-top:12px;font-size:12px;color:var(--green);line-height:1.6;">' +
    '<strong style="color:var(--orange);">ð Reading the trend:</strong> In ' + years[0] + ', only <strong>' + firstPct.toFixed(0) + '%</strong> of homes sat 60+ days. By ' + years[years.length-1] + ', that number is <strong>' + lastPct.toFixed(0) + '%</strong>. ';
  if (lastPct > firstPct + 10) {
    html += 'A meaningful rise â sellers should expect to offer more concessions to close deals.';
  } else if (lastPct > firstPct) {
    html += 'Slight increase in buyer leverage â concession asks more common but not aggressive.';
  } else {
    html += 'Inventory is moving â buyers have less leverage to demand concessions.';
  }
  html += '</div>';
  html += '</div>';
...
function renderFloorPlans() {
  ...
  var html = '<div style="font-size:11px;color:var(--muted);margin-bottom:10px;">Showing <strong>' + models.length + '</strong> Del Webb floor plans Â· click any card to see all sales of that model</div>';
  ...
  models.forEach(function(m){
    html += '<div class="fp-card" onclick="fpJumpToModel(\'' + m.key.replace(/'/g, "\\'") + '\')">' +
      ...
      '<div class="fp-card-stats">' +
        '<div>ð ~' + m.avgSqft.toLocaleString() + ' sf Â· built ' + m.avgYr + '</div>' +
        '<div>ð° ' + miFmtMoney(m.medianPrice) + ' median Â· $' + m.avgPsf + '/sf</div>' +
        '<div>ð ' + m.count + ' sale' + (m.count===1?'':'s') + ' Â· ' + m.avgDom + 'd avg DOM</div>' +
      '</div>' +
      '<div class="fp-card-cta">View Sales â</div>' +
    '</div>';
  });
  html += '</div>';
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:12px;padding:18px 22px;margin-top:18px;">' +
    '<div style="font-family:\'Merriweather\',serif;font-size:1.1rem;font-weight:700;margin-bottom:6px;">ð Looking for the actual floor plan diagrams?</div>' +
    '<div style="font-size:12px;opacity:.9;line-height:1.5;margin-bottom:12px;">' +
      'Lona\'s comprehensive Sun City Festival floor plan library has every Del Webb model with diagrams, sq ft breakdowns, and key features. Currently in development at lonajking.com.' +
    '</div>' +
    '<a href="https://lonajking.com" target="_blank" style="display:inline-block;padding:9px 18px;background:var(--orange);color:#fff;border-radius:6px;font-family:inherit;font-size:12px;font-weight:700;text-decoration:none;">Visit lonajking.com â</a>' +
  '</div>';
...
function renderQuarterlyReport() {
  ...
  html += '<div style="border-bottom:3px solid var(--orange);padding-bottom:14px;margin-bottom:24px;">' +
    '<div style="display:flex;justify-content:space-between;align-items:flex-start;">' +
      '<div>' +
        '<div style="font-family:\'Merriweather\',serif;font-size:1.7rem;font-weight:800;color:var(--green);">Sun City Festival Market Report</div>' +
        '<div style="font-size:13px;color:var(--muted);margin-top:4px;">' + periodLabel + ' Â· Generated ' + new Date().toLocaleDateString('en-US', {month:'long', day:'numeric', year:'numeric'}) + '</div>' +
      '</div>' +
      '<div style="text-align:right;font-size:11px;color:var(--muted);line-height:1.5;">' +
        ...
        'HomeSmart Â· Sun City Festival<br>' +
        '(623) 203-6271 Â· lona@hometownaz.com<br>' +
        'hometownaz.com' +
      '</div>' +
    '</div>' +
  '</div>';
  ...
  if (pyAgg) {
    ...
    html += '<div class="qr-section">' +
      '<h3>ð Year-Over-Year Performance</h3>' +
      '<div class="qr-grid">' +
        ...
        '<div class="qr-stat"><div class="qr-stat-lbl">Days on Market</div>' +
          '<div class="qr-stat-val" style="color:' + color(domDelta, true) + ';">' + (domDelta > 0 ? 'â +' : (domDelta === 0 ? 'â ' : 'â ')) + Math.round(domDelta) + ' d</div>' +
  ...
  // Top 5 sales
  html += '<div class="qr-section">' +
    '<h3>ð Top 5 Sales This Period</h3>' +
    '<table style="width:100%;font-size:12px;border-collapse:collapse;">' +
  ...
  top5.forEach(function(r){
    html += '<tr><td style="padding:8px;border-bottom:1px solid #f0ebe0;">' + r.addr + '</td>' +
      ...
      '<td style="padding:8px;border-bottom:1px solid #f0ebe0;font-size:11px;color:var(--muted);">' + (r.model || r.name || 'â') + '</td>' +
    '</tr>';
  });
  html += '</tbody></table></div>';
  ...
  // Top models
  html += '<div class="qr-section">' +
    '<h3>ð Most Popular Floor Plans This Period</h3>' +
    '<table style="width:100%;font-size:12px;border-collapse:collapse;">' +
      '<thead><tr style="background:#fdf9f1;text-align:left;">' +
        '<th style="padding:8px;border-bottom:1px solid var(--border);">Model</th>' +
  ...
  html += '<div style="background:#1d3a2c;color:#fff;border-radius:10px;padding:20px 24px;margin-top:24px;">' +
    '<div style="font-family:\'Merriweather\',serif;font-size:1.1rem;font-weight:700;margin-bottom:6px;color:var(--orange);">Ready to make your move in Sun City Festival?</div>' +
    '<div style="font-size:12px;line-height:1.6;opacity:.9;margin-bottom:10px;">' +
      'This report is generated from real ARMLS closed sales data â not estimates. Lona King has 20+ years of Sun City Festival experience and was Senior Contract Director for Del Webb 1993-2004. ' +
      'Billy Heinzman handles the data, the tech, and the marketing strategy. Together, they\'re Sun City Festival\'s most data-driven team.' +
    '</div>' +
    '<div style="font-size:12px;font-weight:700;">ð (623) 203-6271 Â· âï¸ lona@hometownaz.com Â· ð hometownaz.com</div>' +
  '</div>';

  // Methodology footnote
  html += '<div style="font-size:10px;color:var(--muted);margin-top:14px;font-style:italic;text-align:center;">' +
    'Source: ARMLS closed sales data, Sun City Festival (zip 85396). Period: ' + periodLabel + '. ' +
...
// â RECENT CLOSINGS â dedicated tab view
...
function renderClosings() {
  ...
  // ââ Stats for the windowed set ââ
  function median(arr) {
  ...
  // ââ 24-month trend from MONTHLY_HISTORY ââ
  var trendHtml = '';
  if (typeof MONTHLY_HISTORY !== 'undefined') {
    ...
    if (recentMonths.length >= 2) {
      ...
      // X-axis labels â every 3 months
      ...
      trendHtml = '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px 18px;margin-bottom:14px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;flex-wrap:wrap;gap:8px;">' +
          '<div>' +
            '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--orange);font-weight:700;">ð 24-Month Closings Trend</div>' +
          ...
          '<div style="text-align:right;">' +
            ...
            '</div>' +
            '<div style="font-size:10px;color:var(--muted);">' + firstP.ym + ' â ' + lastP.ym + '</div>' +
          '</div>' +
        '</div>' +
        '<svg viewBox="0 0 ' + W + ' ' + H + '" style="width:100%;height:auto;display:block;">' +
          ...
          '<text x="' + (W - pad.r - 200) + '" y="' + (H - 8) + '" font-size="9" fill="var(--muted)">' +
            '<tspan fill="var(--orange)">â</tspan> median sold price &nbsp;' +
            '<tspan fill="#ddd6c8">â®</tspan> monthly closings' +
          '</text>' +
        '</svg>' +
      '</div>';
    }
  ...
  var statsHtml = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:14px;">' +
    ...
    statTile('% of Ask', median(ratios) ? (median(ratios)*100).toFixed(1) + '%' : 'â', 'median ratio') +
  ...
  var rows = windowed.slice(0, 250).map(function(c){
    var ratioPct = c.soldRatio ? (c.soldRatio * 100).toFixed(1) : 'â';
    var ratioColor = '#666';
    if (c.soldRatio) {
      ratioColor = c.soldRatio < 0.95 ? '#c8662a' : (c.soldRatio >= 1.0 ? '#2e7d32' : '#666');
    }
    var dateShort = c.closeDate ? c.closeDate.substring(5) : 'â';
    return '<tr style="border-bottom:1px solid #f0ebe0;">' +
      '<td style="padding:8px 10px;font-weight:600;font-size:12px;">' + (c.addr||'â') + '</td>' +
      ...
      '<td style="padding:8px 10px;text-align:center;font-size:11px;">' + (c.beds||'â') + '/' + (c.baths||'â') + '</td>' +
      ...
      '<td style="padding:8px 10px;text-align:center;font-size:11px;">' + (c.model||'â') + '</td>' +
      '<td style="padding:8px 10px;text-align:center;font-size:11px;color:var(--muted);">' + (c.yr||'â') + '</td>' +
      '<td style="padding:8px 10px;text-align:center;font-size:11px;">' + (c.dom||0) + 'd</td>' +
    '</tr>';
  }).join('');
  
  var truncNote = windowed.length > 250 ? 
    '<div style="font-size:10.5px;color:var(--muted);font-style:italic;margin-top:8px;text-align:center;">Showing first 250 of ' + windowed.length + ' â narrow your search or shorten the window to see more</div>' : '';
  ...
  var tableHtml = '<div style="background:#fff;border:1px solid var(--border);border-radius:10px;padding:16px 18px;">' +
    ...
    '<div style="font-size:10.5px;color:var(--muted);margin-top:10px;font-style:italic;line-height:1.5;">' +
      ...
      '<span style="color:#2e7d32;font-weight:700;">Green â¥100%</span> means sold at or over original ask.' +
    '</div>' +
  '</div>';
