...
function renderHeaderKpis(mode) {
  ...
  var kpis = [
    ...
    {n: miFmtMoney(totalVol),              lbl: 'Total Volume',                  tip: 'Cumulative dollar value of every sale tracked. The size of The Grand housing economy.'}
  ];
...
function miNormalizeModel(s) {
  ...
  if (lower === 'sun city grand' || lower === 'del webb' || lower === '--') return '';
...
function miLeadCaptureHtml(context) {
  return '<div class="mi-lead-card">' +
    ...
    '<div class="mi-lead-sub">' +
      'Get a customized The Grand market report — including these results, plus comparable sales detail, and Lona\'s expert read on what it means for ' +
...
function miSubmitLead(ctx) {
  ...
  var subject = 'The Grand Market Report Request — ' + name;
...
function renderCrystalBall() {
  ...
  html += '<div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px;">' +
    '<div style="font-size:13px;font-weight:700;color:var(--green);margin-bottom:8px;">📊 Median Sale Price Trajectory — The Grand (with ' + nextY + ' projection)</div>' +
    svg +
  '</div>';
...
function renderPropHistory() {
  ...
  if (!q || q.length < 3) {
    ...
    resEl.innerHTML = '<div style="background:#fdf9f1;border:1px dashed var(--border);border-radius:10px;padding:24px;text-align:center;color:var(--muted);">' +
      '<div style="font-size:2rem;margin-bottom:8px;">🏘️</div>' +
      '<div style="font-size:13px;">Type any The Grand address to see its complete sale history</div>' +
...
function renderStreetDetail(s) {
  ...
  if (typeof document !== 'undefined' && document.title) {
    document.title = s.name + ' — The Grand Real Estate Profile | Lona King & Billy Heinzman';
  }
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:14px;padding:24px 28px;margin-bottom:14px;position:relative;overflow:hidden;">' +
    ...
    '<div style="position:relative;">' +
      '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--orange);font-weight:800;margin-bottom:4px;">📍 The Grand Street Profile</div>' +
...
function renderLeaderboard() {
  ...
  resEl.innerHTML = '<div class="coming-soon-card">' +
    ...
    '<div class="coming-soon-sub">' +
      'Once we wire in <strong>Agent Productivity</strong> data from ARMLS, this tab will show the top-performing The Grand listing agents ranked by:' +
    '</div>' +
    ...
    '<div style="margin-top:18px;font-size:11px;color:var(--orange);font-weight:700;">' +
      '💡 Once enabled, this tool will be <em>the</em> reason every The Grand agent visits this site weekly.' +
    '</div>' +
  '</div>';
}
...
function renderFloorPlans() {
  ...
  html += '<div style="background:linear-gradient(135deg,var(--green),#3a5040);color:#fff;border-radius:12px;padding:18px 22px;margin-top:18px;">' +
    ...
    '<div style="font-size:12px;opacity:.9;line-height:1.5;margin-bottom:12px;">' +
      'Lona\'s comprehensive The Grand floor plan library has every Del Webb model with diagrams, sq ft breakdowns, and key features. Currently in development at lonajking.com.' +
    '</div>' +
...
function renderQuarterlyReport() {
  ...
  html += '<div style="border-bottom:3px solid var(--orange);padding-bottom:14px;margin-bottom:24px;">' +
    '<div style="display:flex;justify-content:space-between;align-items:flex-start;">' +
      '<div>' +
        '<div style="font-family:\'Merriweather\',serif;font-size:1.7rem;font-weight:800;color:var(--green);">The Grand Market Report</div>' +
  ...
  html += '<div style="background:#1d3a2c;color:#fff;border-radius:10px;padding:20px 24px;margin-top:24px;">' +
    '<div style="font-family:\'Merriweather\',serif;font-size:1.1rem;font-weight:700;margin-bottom:6px;color:var(--orange);">Ready to make your move in The Grand?</div>' +
    '<div style="font-size:12px;line-height:1.6;opacity:.9;margin-bottom:10px;">' +
      'This report is generated from real ARMLS closed sales data — not estimates. Lona King has 20+ years of The Grand experience and was Senior Contract Director for Del Webb 1993-2004. ' +
      'Billy Heinzman handles the data, the tech, and the marketing strategy. Together, they\'re The Grand\'s most data-driven team.' +
    '</div>' +
  ...
  html += '<div style="font-size:10px;color:var(--muted);margin-top:14px;font-style:italic;text-align:center;">' +
    'Source: ARMLS closed sales data, The Grand (zip 85375). Period: ' + periodLabel + '. ' +
